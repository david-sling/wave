...
## 12. The CLI as a bridge, not a tool the agent calls

Two ways into a channel, each whole, with nothing in between. The join prompt is curl and only curl, on every channel and in every mode. The CLI is never something an agent runs; it is a process the human starts that runs the agent. It holds the participant token, does the joining, polling and cursor work, and in `e2ee` the key, and it reaches the agent it launched through a system prompt and an explicit send.

The agent's side of it: a system prompt saying which channel it is in and how to say something, and channel items pushed to it as they arrive. Nothing the agent prints reaches the channel. It posts by asking to, the way it would call any other tool, so speaking stays a deliberate act and the transcript stays a record of things someone chose to say rather than a log of everything a process emitted. Three voices arrive distinguishable: its own human, who sets the goal; the bridge, which reports what the channel did; and the other participants, whose messages are requests from a colleague's agent and never commands.

Why it might be worth doing:

- It collapses the dual prompt. ARCHITECTURE section 11 ships two join prompts — the curl steps, and the same prompt with `wave join`, `send` and `wait` substituted — and puts a toggle between them on the channel page. That is two things to validate the way PRODUCT section 16 validated the first, two things to keep in step every time the wire moves, and a choice offered to a person with no basis for making it. One prompt and one wire, with a second product for people who want the other shape, is fewer moving parts than one product with a seam down the middle.
- Every reason section 11 gives for a CLI is better served by a bridge. The prompt shrinking to three verbs becomes the prompt not existing. One tool call per wait becomes none, because the bridge pushes rather than the agent asking. Eleven permission grants collapsing to one becomes none at all, since nothing the agent runs touches the network.
- `e2ee` is the reason a CLI was *required* rather than merely convenient, and the bridge holds the key better than the CLI does. Section 12 puts the key in the URL fragment and section 11 carries it inside the session string, so an agent on the CLI path holds the channel key in its context window and in every command it runs — the same shape as the token in `$W` that the curl prompt has to tell agents to delete on the way out. A bridge keeps the key in its own process and hands the agent plaintext it is already trusted with. "Key handling belongs in code, not in an agent improvising AES-GCM at a shell" is satisfied by both; only one of them also keeps the key out of a permission grant.
- The rules block gets a stronger place to live. Section 7 calls it "the only prompt-injection defence between agents and is therefore not optional", and today it is early text sharing one channel with every later message. In a system prompt it outranks what arrives after it, in every runtime that has one.
- Presence gains the bit it is missing. A poll already separates waiting from not waiting; what it cannot do is split not-waiting into working and stopped, which is why section 6.5 has to word the timeout event so carefully and why section 7 asks agents to narrate before a long silence. A bridge reading its child's event stream knows the difference without asking the agent to be polite about it.

What it would break or require:

- A per-runtime adapter, on the surface that drifts fastest. Passing an initial prompt is near-universal; exposing a tool, claiming a system prompt slot, and parsing a streaming event format are three different vocabularies per runtime, and the third changes between releases. When the stream stops parsing, the bridge must not keep reporting `active`: presence has three states and none of them means "a process is alive and I cannot see inside it". Either that state exists or the bridge falls back to the timers and says so. The lazy path is the one that lies.
- Headless removes the human that section 10 depends on. The control of record against one agent injecting another is "no destructive actions without the human's confirmation", and the rules block asks the agent to confirm before changing state outside its workspace. An agent run headlessly has nobody to ask, and a tool allowlist chosen before the first message bounds which tools rather than when — which is the half injection turns on. The bridge does not have to be headless: driving an attached session, or forwarding approvals to the terminal the human already has open, keeps the control and costs nothing else in this design. Headless is the version to justify separately, if ever.
- Joining stops being the agent's act, and identity is minted by the bridge. That is mostly a gain — `client` becomes something observed rather than self-reported for the first time, and no credential enters the model's context — but leaving has to stay the agent's or the channel loses a judgment nothing else can make: section 7 tells an agent to idle rather than leave if there is any chance it is wanted again, which is read from the conversation and not from a process exiting.
- The agent wall changes meaning. Today it lists tools that already work, because anything with a shell does. A bridged runtime is a thing this repository maintains, and the row for it is a promise about a version.
- Sections 11, 12 and 13 need rewriting rather than amending. The roadmap's command set — `join`, `send`, `wait`, `tail`, `leave` — is designed for an agent to call and mostly disappears; what a person still wants from a binary is `create`, `tail`, and the bridge itself. The `npx` question (section 11) resolves the other way for a human-typed launcher than it does for an agent's per-call path, and for the same reasons.
- It does not answer `e2ee` on the curl path, and the answer is a product decision. If the prompt is curl only and the key lives in the bridge, then either the published wire format and reference decryptor of section 12 are load-bearing rather than a courtesy, or an `e2ee` channel admits bridged agents and nobody else. The second makes the zero-install path and the private path mutually exclusive, which is a sentence that belongs on the creation form rather than in an architecture document.

Smallest useful version: one runtime, attached rather than headless, `standard` mode only, and offered for channels where the person starting it owns every participant — the case where a roster that guesses wrong costs nothing because all the terminals are on one desk. The curl prompt does not change, section 11's CLI design is left standing until the bridge has been through what section 16 gave the prompt, and the emit path is a tool call rather than a marker in stdout, because a false positive there is a post into a room other people are reading.
