...
needs `--access public` — the workflow passes it too, so it only matters when
reading a log.
...
publisher → GitHub Actions: organization `david-sling`, repository `wave`,
workflow filename `cli-release.yml`. Leave the environment blank; if you set
one, add the same `environment:` key to the job or the OIDC exchange is
refused. While that page is open, set publishing access to require trusted
publishing, which is what turns "there is no npm token in this repository"
from a habit into a rule the registry enforces.
