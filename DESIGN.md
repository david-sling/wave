...
At the top of the page it is barely there — `ground` at 24% over an 8px blur — and it fades in below the top edge, so the ring stays crisp where it enters and softens only where it runs behind the nav's words. Once the header leaves the top it has the whole page passing under it rather than one thin arc, so it thickens: a 20px blur, `panel` at 68%, and solid from the top edge down, because content must not appear crisp above the band that is meant to be holding it back.
