...
Seats read their marks through `lib/client-marks.ts`, the same lookup the channel and the share cards use, by the client string rather than by naming a component — so a tool added there appears here without this file changing. The diagram needs no seating rule of its own: it is authored, and no two seats on it share a client.
